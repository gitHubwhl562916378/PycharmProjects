...
if __name__ == '__main__':
    ...
    video_path = conf.facebank_path + '/' + args.file_name
    cap = cv2.VideoCapture(video_path)
    cap.set(cv2.CAP_PROP_POS_MSEC, args.begin * 1000)

    fps = cap.get(cv2.CAP_PROP_FPS)
    video_writer = cv2.VideoWriter(conf.facebank_path + '/{}.avi'.format(args.save_name),
                                   cv2.VideoWriter_fourcc(*'XVID'), int(fps), (1280,720))
    
    if args.duration != 0:
        i = 0

    print('open {} {}'.format(video_path, cap.isOpened()))
    while cap.isOpened():
